...
<template>
  <div style="display: grid; gap: 24px">
    ...
    <section class="hero-gradient glass-card" style="padding: 34px; overflow: hidden">
      <div class="responsive-grid" style="grid-template-columns: 1.2fr 0.8fr; align-items: center">
        ...
        <div class="section-card" style="padding: 22px; background: rgba(255,255,255,0.14); border-color: rgba(255,255,255,0.16); color: #fff">
    ...
    <SectionCard title="热门分类" description="快速进入重点图书品类。">
      <div v-loading="loading" class="book-grid" style="grid-template-columns: repeat(auto-fill, minmax(180px, 1fr))">
        <div v-for="category in categories" :key="category.id" class="section-card" style="padding: 20px; cursor: pointer" @click="router.push({ path: '/books', query: { categoryId: category.id } })">
          <div class="cover-placeholder" style="width: 52px; height: 52px; border-radius: 18px; margin-bottom: 18px">分</div>
          <div style="font-size: 18px; font-weight: 700">{{ category.name }}</div>
          <div style="margin-top: 8px; color: var(--text-secondary)">{{ category.children?.length || 0 }} 个子分类</div>
        </div>
      </div>
    </SectionCard>
    ...
    <SectionCard title="精选图书" description="直接进入详情或加入购物车。">
      <div v-if="books.length" class="book-grid" v-loading="loading">
        <article v-for="book in books" :key="book.id" class="section-card" style="padding: 16px; display: grid; gap: 14px">
          ...
          <div>
            <div style="font-size: 18px; font-weight: 700; line-height: 1.4">{{ book.name }}</div>
